...
AILogic.prototype.init =  function (gameRule){
     this.gameRule=gameRule;
     this.cardList=this.player.cardList;
    this.cards = [];
    for(var j=0;j<this.cardList.length;j++){
        var cd={};
        cd.val=this.gameRule.pkpx[this.cardList[j]-1];
        cd.xh=this.cardList[j];
        cd.type=this.gameRule.pllx[this.cardList[j]-1];
        this.cards.push(cd);
    }
...
AILogic.prototype.yuchuli =  function (gameRule){
    this.cards = [];
    this.cardList=this.player.cardList;
    for(var j=0;j<this.cardList.length;j++){
        var cd={};
...
AILogic.prototype.follow = function(winc, isWinnerIsLandlord, winnerCardCount) {
    var self = this;
    self.log();
    console.log('buchongRoom follow:',winc.cardKind);
    this.yuchuli();
   var result = (function(){
        switch (winc.cardKind) {//判断牌型
            case self.gameRule.ONE://单牌
                var one = self.matchCards(self._one, self.gameRule.ONE, winc, isWinnerIsLandlord, winnerCardCount);
                console.log('buchongRoom:one  ',one);
                if(!one){
                ...
                return one;
            case self.gameRule.PAIRS://对子
                var pairs =  self._pairs.length > 0 ? self.matchCards(self._pairs, self.gameRule.PAIRS, winc, isWinnerIsLandlord, winnerCardCount) : null;
                ...
                return pairs;
            case self.gameRule.THREE://三根
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                return self.matchCards(self._three, self.gameRule.THREE, winc, isWinnerIsLandlord, winnerCardCount);

            case self.gameRule.THREE_WITH_ONE://三带一
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                var three = self.minCards(self._three, self.gameRule.THREE, winc.val);
                if(three){
                    ...
                    three.cardKind = self.gameRule.THREE_WITH_ONE;
                    three.size = 4;
                }
                return three;

            case self.gameRule.THREE_WITH_PAIRS: //三带一对
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                var three = self.minCards(self._three, self.gameRule.THREE, winc.val);
                if(three){
                    var pairs = self.minCards(self._pairs, self.gameRule.PAIRS);
                    while (true) {//避免对子三根重叠
                        if(pairs.cardList[0].val === three.val){
                            pairs = self.minCards(self._pairs, self.gameRule.PAIRS, pairs.cardList[0].val);
                        } else {
                            break;
                        }
                    }
                    ...
                    three.cardKind = self.gameRule.THREE_WITH_PAIRS;
                    three.size = 5;
                }
                return three;

            case self.gameRule.PROGRESSION://顺子
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                    ...
                    for (var i = self._progression.length - 1; i >= 0 ; i--) {//从小值开始判断
                        if(winc.val < self._progression[i].val && winc.size <= self._progression[i].cardList.length){
                            if(winc.size === self._progression[i].cardList.length){
                                return self.setCardKind(self._progression[i], self.gameRule.PROGRESSION);
                            } else {
            ...
            case self.gameRule.PROGRESSION_PAIRS://连对
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                if(self._progressionPairs.length > 0){
                    for (var i = self._progressionPairs.length - 1; i >= 0 ; i--) {//从小值开始判断
                        if(winc.val < self._progressionPairs[i].val && winc.size <= self._progressionPairs[i].cardList.length){
                            if(winc.size === self._progressionPairs[i].cardList.length){
                                return self.setCardKind(self._progressionPairs[i], self.gameRule.PROGRESSION_PAIRS);
                            } else {
                                if(self.player.isLandlord || isWinnerIsLandlord){
            ...
            case self.gameRule.PLANE://三顺
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                return self.minPlane(winc.size, winc);
            case self.gameRule.PLANE_WITH_ONE: //飞机带单
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                var cnt = winc.size / 4,
                ...
                if(plane){
                    ...
                    plane.cardKind = self.gameRule.PLANE_WITH_ONE;
                    plane.size = plane.cardList.length;
                }
                return plane;
            case self.gameRule.PLANE_WITH_PAIRS://飞机带对
                if(!isWinnerIsLandlord && !self.player.isLandlord){
                    return null;
                }
                var cnt = winc.size / 5,
                ...
                if(plane){
                    var currPairsVal = 2;
                    for (var i = 0; i < cnt; i++) {
                        var pairs = self.minCards(self._pairs, self.gameRule.PAIRS, currPairsVal);//拿一对
                        if(pairs){
                    ...
                    plane.cardKind = self.gameRule.PLANE_WITH_PAIRS;
                    plane.size = plane.cardList.length;
                }
                return plane;

            case self.gameRule.BOMB://炸弹
                if(!isWinnerIsLandlord && !self.player.isLandlord){//同是农民不压炸弹
                    return null;
                }
                var bomb = self.minCards(self._bomb, self.gameRule.BOMB, winc.val);
                if(bomb){
                    return bomb;
                } else {
                    if(self._kingBomb.length > 0){
                        if((isWinnerIsLandlord && winnerCardCount < 6)
                            || (self.player.isLandlord && self.player.cardList.length < 6)||(winc.val>13)){
                            return self.setCardKind(self._kingBomb[0], self.gameRule.KING_BOMB);
                        }
                    }
                    return null;
                }
            case self.gameRule.FOUR_WITH_TWO:
                return self.minCards(self._bomb, self.gameRule.BOMB, winc.val);
            case self.gameRule.FOUR_WITH_TWO_PAIRS:
                return self.minCards(self._bomb, self.gameRule.BOMB, winc.val);
            case self.gameRule.KING_BOMB:
                return null;
            default:
                return null;
        }
    ...
    if(result){
        return result;
    } else if(winc.cardKind != self.gameRule.BOMB && winc.cardKind != self.gameRule.KING_BOMB
        ...
        if((isWinnerIsLandlord && winnerCardCount < 5)
            ...
            if(self._bomb.length > 0){
                return self.minCards(self._bomb, self.gameRule.BOMB);
            } else {
                return self.setCardKind(self._kingBomb[0], self.gameRule.KING_BOMB);
            }
        }
    ...
    var cardsWithMin = function (idx){
        ...
        for (var i = 0; i < self._one.length; i++) {
            if(self._one[i].val === minCard.val){
                return self.minCards(self._one, self.gameRule.ONE);
            }
        }
        //对子里找
        for (i = 0; i < self._pairs.length; i++) {
            if(self._pairs[i].val === minCard.val){
                return self.minCards(self._pairs, self.gameRule.PAIRS);
            }
        }
        //三根里找
        for (i = 0; i < self._three.length; i++) {
            if(self._three[i].val === minCard.val){
                return self.minCards(self._three, self.gameRule.THREE);
            }
        }
        //炸弹里找
        ...
        for (i = 0; i < self._plane.length; i++) {
            for (var j = 0; j < self._plane[i].cardList.length; j++) {
                if(self._plane[i].cardList[j].val === minCard.val && self._plane[i].cardList[j].type === minCard.type ){
                    return self.minCards(self._plane, self.gameRule.PLANE);
                }
            }
        }
        //顺子里找
        for (i = 0; i < self._progression.length; i++) {
            for (var j = 0; j < self._progression[i].cardList.length; j++) {
                if(self._progression[i].cardList[j].val === minCard.val && self._progression[i].cardList[j].type === minCard.type ){
                    return self.minCards(self._progression, self.gameRule.PROGRESSION);
                }
            }
        }
        //连对里找
        for (i = 0; i < self._progressionPairs.length; i++) {
            for (var j = 0; j < self._progressionPairs[i].cardList.length; j++) {
                if(self._progressionPairs[i].cardList[j].val === minCard.val && self._progressionPairs[i].cardList[j].type === minCard.type ){
                    return self.minCards(self._progressionPairs, self.gameRule.PROGRESSION_PAIRS);
                }
            }
        }
        if(self._kingBomb.length > 0){
            return self.minCards(self._kingBomb, self.gameRule.KING_BOMB);
        }
    };
    for (var i = self.cards.length - 1; i >=0 ; i--) {
        var r = cardsWithMin(i);
        if(r.cardKind === self.gameRule.ONE){
            if(self._plane.length > 0){//三顺
                var plane = self.minCards(self._plane, self.gameRule.PLANE);
                ...
                return self.setCardKind( plane, self.gameRule.PLANE_WITH_ONE);
            }
            else if(self._three.length > 0){//三带一
                var three = self.minCards(self._three, self.gameRule.THREE);
                var len = three.cardList.length / 3;
                var one = self.minOne(currOneVal, three.val);//拿一张单牌
                three.cardList.push(one);
                if(three.val < 14)
                    return self.setCardKind( three, self.gameRule.THREE_WITH_ONE);
            }
        ...
        } else if(r.cardKind === self.gameRule.THREE){
            var three = self.minCards(self._three, self.gameRule.THREE);
            var len = three.cardList.length / 3;
            if(self._one.length >= 0){//单根多带单
                ...
                var pairs = self.minCards(self._pairs, self.gameRule.PAIRS, currPairsVal);//拿一对
                three.cardList = three.cardList.concat(pairs.cardList);
                return self.setCardKind( three, self.gameRule.THREE_WITH_PAIRS);
            } else {
                return self.setCardKind( three, self.gameRule.THREE);
            }
        } else if(r.cardKind === self.gameRule.PLANE){
            var plane = self.minCards(self._plane, self.gameRule.PLANE);
            var len = plane.cardList.length / 3;
            if(self._one.length > len && self._pairs.length > len){
                if(self._one.length >= self._pairs.length){//单根多带单
                    ...
                    return self.setCardKind( plane, self.gameRule.PLANE_WITH_PAIRS);
                }
                ...
                return self.setCardKind( plane, self.gameRule.PLANE);
            }
        }else if(r.cardKind === self.gameRule.BOMB && self.times() === 1){
            return r;
        } else if(r.cardKind === self.gameRule.BOMB && self.times() != 1){
            continue;
        } else {
...
AILogic.prototype.playOneAtTheEnd  = function(landlordCardsCnt) {
    ...
    if(self._progression.length > 0){//出顺子
        ...
        if((self.player.isLandlord && (self.player.nextPlayer.cardList.length === 2 || self.player.nextPlayer.nextPlayer.cardList.length === 2))
            ...
            return self.maxCards(self._pairs, self.gameRule.PAIRS);
        else
            return self.minCards(self._pairs, self.gameRule.PAIRS);
    }
        ...
        if((self.player.isLandlord && (self.player.nextPlayer.cardList.length <= 2 || self.player.nextPlayer.nextPlayer.cardList.length <= 2))
            || (!self.player.isLandlord && landlordCardsCnt <= 2))
            return self.maxCards(self._one, self.gameRule.ONE);
        else
            return self.minCards(self._one, self.gameRule.ONE);
    } else {//都计算无结果出最小的那张牌
        var one = null;
...
AILogic.prototype.prompt = function (winc){
    var self = this,
        stat = self.gameRule.valCount(self.cards);
    ...
    if(winc){//跟牌
        ...
        switch (winc.cardKind) {
            ...
            case self.gameRule.THREE_WITH_ONE://三带一
                ...
                break;
            case self.gameRule.THREE_WITH_PAIRS://三带对
                var threePrompt = setPrompt(3, winc.val, stat.slice(0)),
            ...
            case self.gameRule.PLANE://三顺
                ...
                break;
            case self.gameRule.PLANE_WITH_ONE:
                promptList = getPlanePrompt(winc.size / 4);
                break;
            case self.gameRule.PLANE_WITH_PAIRS:
            ...
            case self.gameRule.FOUR_WITH_TWO_PAIRS:
                promptList = setPrompt(4, winc.val, stat);
                break;
            case self.gameRule.BOMB:
                promptList = setPrompt(4, winc.val, stat);
                break;
            default:
                break;
        }
        if(winc.cardKind != self.gameRule.KING_BOMB && winc.cardKind != self.gameRule.BOMB){
            //炸弹加入
            if(self._bomb.length > 0){
        ...
        if(winc.cardKind != self.gameRule.KING_BOMB){
            //王炸加入
            if(self._kingBomb.length > 0){
...
AILogic.prototype.getMinVal = function(n, v){
    var self = this,
        c = self.gameRule.valCount(self.cards);
...
AILogic.prototype.analyse = function(){
    ...
    target.sort(self.gameRule.cardSort);
    //判定王炸
    if(self.gameRule.isKingBomb(target.slice(0,2))){
        self._kingBomb.push(new qc.landlord.AICardType(17, target.splice(0, 2)));
    }
    //判定炸弹
    stat = self.gameRule.valCount(target);
    ...
    self._three = [];
    targetWobt.sort(self.gameRule.cardSort);
    //判定顺子，先确定五连
    ...
    //除去顺子、炸弹、三根后判断对子、单牌
    stat = self.gameRule.valCount(targetWobp);
...
AILogic.prototype.judgeThree = function (cards){
    var self = this,
        stat = self.gameRule.valCount(cards);
    for (i = 0; i < stat.length; i++) {
        if(stat[i].count === 3){
            var list = [];
...
var qc={};
qc.landlord={};
qc.landlord.AICardType= function(v, list){
    this.val = v;
    this.cardList = list;
};
/**
...
AILogic.prototype.judgeProgressionPairs = function (cards){
    ...
    var saveProgressionPairs = function (proList){
        ...
        progressionPairs.sort(self.gameRule.cardSort);
        self._progressionPairs.push(new qc.landlord.AICardType(proList[0], progressionPairs));
    };
    //判定连对
    if(cards.length >= 6){
        var proList = [];
        var stat = self.gameRule.valCount(cards);//统计
        for (var i = 0; i < stat.length; i++) {
        ...
        for (var i = self._plane.length - 1; i >= 0 ; i--) {//从小值开始判断
            if(winc.val < self._plane[i].val && len <= self._plane[i].cardList.length){
                if(len === self._plane[i].cardList.length){
                    return self.setCardKind(self._plane[i], self.gameRule.PLANE);
                } else {
...
AILogic.prototype.minOne = function (v, notEq){
    var self = this,
        one = self.minCards(self._one, self.gameRule.ONE, v),
...
AILogic.prototype.offPairs = function (v, notEq){
    var self = this,
        pairs = self.minCards(self._pairs, self.gameRule.PAIRS, v);
    if(pairs){
        while (true) {
